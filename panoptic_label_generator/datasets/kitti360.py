...
class Kitti360(Dataset):
    CLASS_COLOR_14 = np.zeros((256, 3), dtype=np.uint8)
    CLASS_COLOR_14[:14, :] = np.array([
        [128, 64, 128],
        [244, 35, 232],
        [70, 70, 70],
        [190, 153, 153],
    ...
    def __init__(
        ...
        super().__init__("kitti_360", ["train", "val", "sequence"], mode, cfg, transform,
        ...
        print(f"Found {len(self.frame_paths)} frames in {self.mode} mode")
    ...
    def __getitem__(
        ...
        if not (self.return_only_rgb or return_only_rgb):
            ...
            if not self.frame_paths[index]["semantic"] is None and \
                    not self.frame_paths[index]["instance"] is None:
                ...
                # small_instance_area = self.small_instance_area_full_res * \
                #                       (height / full_res_h) * (width / full_res_w)
                #
        ...
        # For saving predictions in the KITTI-360 format
        semantic_path = self.frame_paths[index]["semantic"]
        instance_path = self.frame_paths[index]["instance"]
        output.update({
    ...
    def _convert_semantics(self, semantic: ArrayLike) -> ArrayLike:
        ...
        elif self.label_mode == "cityscapes_19":
            # https://github.com/autonomousvision/kitti360Scripts/blob/master/kitti360scripts/helpers/labels.py
...
class Kitti360DataModule(pl.LightningDataModule):
    ...
    def __init__(self, cfg_dataset, num_classes: int, batch_size: int, num_workers: int,
                 transform_train: List[Callable], transform_test: List[Callable], label_mode: str,
                 train_sample_indices: List[int] = None, test_sample_indices: List[int] = None):
        super().__init__()
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            self.kitti360_train = Kitti360(
                self.cfg_dataset.train_split, self.cfg_dataset,
                transform=self.transform_train,
                return_only_rgb=False,
                sequences=self.cfg_dataset.train_sequences,
                sequence_reference_mode=self.cfg_dataset.sequence_reference_mode,
                label_mode=self.label_mode,
                subset=self.train_sample_indices)
            assert self.kitti360_train.num_classes == self.num_classes
        if stage == "validate" or stage is None:
            pass

        # Assign test dataset for use in dataloader(s)
        if stage == "test" or stage is None:
            self.kitti360_test = Kitti360(
                self.cfg_dataset.val_split, self.cfg_dataset,
                transform=self.transform_test,
                return_only_rgb=False,
                sequences=self.cfg_dataset.val_sequences,
                sequence_reference_mode=self.cfg_dataset.sequence_reference_mode,
                label_mode=self.label_mode,
                subset=self.test_sample_indices)
            assert self.kitti360_test.num_classes == self.num_classes

        if stage == "predict" or stage is None:
            pass

    def train_dataloader(self):
        return DataLoader(self.kitti360_train, batch_size=self.batch_size, shuffle=True,
                          num_workers=self.num_workers, pin_memory=False)

    def test_dataloader(self):
        return DataLoader(self.kitti360_test, batch_size=self.batch_size, shuffle=False,
                          num_workers=self.num_workers, pin_memory=False)
