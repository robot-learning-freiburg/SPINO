    ...
    def load_state_dict(self, state_dict):
        for k, v in state_dict.items():
            if k in self._states:
                ...
                raise KeyError(f"Unexpected key {k} in state dict when loading "
                               f"{self.__class__.__name__} from state dict")
...
class ConfusionMatrixMeter(AverageMeter):
    ...
    def __init__(self, num_classes, momentum=1.):
        super().__init__((num_classes, num_classes), momentum)
    ...
    def iou(self):
        if self.count.item() == 0:
            return torch.tensor(0.)
        mean_conf = self.mean
        return mean_conf.diag() / (
            (mean_conf.sum(dim=0) + mean_conf.sum(dim=1) - mean_conf.diag()) + 1e-8)
