...
# -------------------------------------------------------- #
# Adapted from
# https://github.com/bowenc0221/panoptic-deeplab/blob/master/segmentation/model/post_processing/semantic_post_processing.py
...
def find_instance_center(ctr_hmp, threshold=0.1, nms_kernel=3, top_k=None):
    ...
    Arguments:
        ctr_hmp: A Tensor of shape [N, 1, H, W] of raw center heatmap output, where N is the batch
            size, for consistent, we only support N=1.
        threshold: A Float, threshold applied to center heatmap score.
        nms_kernel: An Integer, NMS max pooling kernel size.
        top_k: An Integer, top k centers to keep.
    Returns:
        A Tensor of shape [K, 2] where K is the number of center points. The order of second dim
            is (y, x).
    """
    if ctr_hmp.size(0) != 1:
        raise ValueError("Only supports inference for batch size = 1")
...
def group_pixels(ctr, offsets):
    ...
    Arguments:
        ctr: A Tensor of shape [K, 2] where K is the number of center points. The order of second
            dim is (y, x).
    ...
    y_coord = torch.arange(height, dtype=offsets.dtype,
                           device=offsets.device).repeat(1, width, 1).transpose(1, 2)
...
def get_instance_segmentation(
    ...
    Arguments:
        ...
        nms_kernel: An Integer, NMS max pooling kernel size.
        top_k: An Integer, top k centers to keep.
        thing_seg: A Tensor of shape [1, H, W], predicted foreground mask, if not provided,
            inference from semantic prediction.
    Returns:
        A Tensor of shape [1, H, W] (to be gathered by distributed data parallel).
        A Tensor of shape [1, K, 2] where K is the number of center points. The order of second dim
            is (y, x).
    """
...
def merge_semantic_and_instance(
    ...
    """
    Post-processing for panoptic segmentation, by merging semantic segmentation label and class
        agnostic instance segmentation label.
    Arguments:
        sem_seg: A Tensor of shape [1, H, W], predicted semantic label.
    ...
    # We use -1 as the new void_label in the panoptic map
    pan_seg = -torch.ones_like(sem_seg, dtype=torch.int16)
    thing_seg = ins_seg > 0
    ...
    for ins_id in instance_ids:
        ...
        if class_id.item() in class_id_tracker:
            ...
            new_ins_id = 1
        class_id_tracker[class_id.item()] += 1
        pan_seg[thing_mask] = class_id.type(pan_seg.dtype) * label_divisor + new_ins_id

    # paste stuff to unoccupied area
    class_ids = torch.unique(sem_seg)
    for class_id in class_ids:
        if class_id.item() in thing_list or class_id.item() == void_label:
            continue
        # calculate stuff area
        stuff_mask = (sem_seg == class_id) & (~thing_seg)
        area = torch.nonzero(stuff_mask).size(0)
        if area >= stuff_area:
            pan_seg[stuff_mask] = class_id.type(pan_seg.dtype) * label_divisor
...
def get_panoptic_segmentation(
    ...
    Arguments:
        sem: A Tensor of shape [N, C, H, W] of raw semantic output, where N is the batch size, for
            consistent, we only support N=1. Or, a processed Tensor of shape [1, H, W].
        ctr_hmp: A Tensor of shape [N, 1, H, W] of raw center heatmap output, where N is the batch
            size, for consistent, we only support N=1.
        offsets: A Tensor of shape [N, 2, H, W] of raw offset output, where N is the batch size,
            for consistent, we only support N=1. The order of second dim is (offset_y, offset_x).
        ...
        nms_kernel: An Integer, NMS max pooling kernel size.
        top_k: An Integer, top k centers to keep.
        foreground_mask: A Tensor of shape [N, 2, H, W] of raw foreground mask, where N is the batch
            size, we only support N=1. Or, a processed Tensor of shape [1, H, W].
    Returns:
    ...
    if sem.dim() != 4 and sem.dim() != 3:
        raise ValueError(f"Semantic prediction with un-supported dimension: {sem.dim()}.")
    ...
    if foreground_mask is not None:
        if foreground_mask.dim() != 4 and foreground_mask.dim() != 3:
            raise ValueError(f"Foreground prediction with un-supported dimension: {sem.dim()}.")
    ...
    instance, center = get_instance_segmentation(semantic,
                                                 ctr_hmp,
                                                 offsets,
                                                 thing_list,
                                                 threshold=threshold,
    ...
    if do_merge_semantic_and_instance:
        panoptic = merge_semantic_and_instance(semantic, instance, label_divisor, thing_list,
                                           stuff_area, void_label)
    else:
        panoptic = None

    return panoptic, center, instance
